...
$InstallDir = if ($env:INSTALL_DIR) { $env:INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA "Programs\speedtest" }
...
try {
  ...
  if (-not (($UserPath -split ';') -contains $InstallDir)) {
    ...
    Write-Warning "Added $InstallDir to the user PATH. Open a new shell to use it."
  }
